...
tf.app.flags.DEFINE_string("MODEL", "slim", "slim or src models")
...
tf.app.flags.DEFINE_string("CONTENT_LAYERS", "relu2_2",
...
tf.app.flags.DEFINE_string("STYLE_IMAGES", "style.jpg", "Styles to train")
...
if FLAGS.MODEL == 'slim':from inception.slim import resnet_model as model
else:import model
import reader
...
def main(argv=None):
    if FLAGS.CONTENT_IMAGES_PATH:
        content_images = reader.image(
                ...
                shuffle=False,
                crop=False)
	# (image - mean_pixel) /255.
	# input to ResNet,do not use preprocess
        generated_images = model.resnet(content_images + reader.mean_pixel,phase_t=False)
    ...
    images = reader.image(FLAGS.BATCH_SIZE, FLAGS.IMAGE_SIZE,
		    FLAGS.TRAIN_IMAGES_PATH,FLAGS.TRAIN_IMAGES_PATH)
    generated = model.resnet(images)
    #TODO preprocess for vgg ?(-=mean_pixel)
    ...
    loss = FLAGS.STYLE_WEIGHT * style_loss + \
    	   FLAGS.CONTENT_WEIGHT * content_loss + FLAGS.TV_WEIGHT * total_variation_loss(generated)

    global_step = tf.Variable(0, name="global_step", trainable=False)
    train_op = tf.train.AdamOptimizer(1e-3).minimize(loss, global_step=global_step)

    output_format = tf.saturate_cast(tf.concat(0, [generated, images]), tf.uint8)
